    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (uriMatcher.match(uri)) {
            case MESSAGES_ALL_ROWS:
                 ...
                 *
                 * Create a URI from the id of the inserted:
                 * Uri instanceUri = MessageContract.CONTENT_URI(...)
                 *
                 * Make sure to notify any observers of this content!
                 * ContentResolver resolver;
                 * resolver.notifyChange(...)
                 */
                long msgID = db.insert(MESSAGES_TABLE, null, values);
                Uri msgInstanceUri = MessageContract.CONTENT_URI(msgID);

                ContentResolver messageResolver = getContext().getContentResolver();
                messageResolver.notifyChange(msgInstanceUri, null);


                return msgInstanceUri;
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        switch (uriMatcher.match(uri)) {
            case MESSAGES_ALL_ROWS:
                 ...
                 */
                Cursor allMessages = db.query(MESSAGES_TABLE, projection, selection, selectionArgs, null, null, sortOrder);
                ContentResolver currentContent = getContext().getContentResolver();
                allMessages.setNotificationUri(currentContent, uri);
